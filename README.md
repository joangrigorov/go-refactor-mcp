...
`go-refactor-mcp` exposes 9 production-grade refactoring tools over standard STDIO (JSON-RPC):
...
| `extract_function` | `file_path` (string, required)<br>`start_line` (number, required)<br>`end_line` (number, required)<br>`new_func_name` (string, required) | Extracts a highlighted block of code into a new function, resolving input/output variables and respecting local scope shadowing. |
| `extract_interface` | `file_path` (string, required)<br>`struct_name` (string, required)<br>`interface_name` (string, required)<br>`dest_file_path` (string, optional) | Analyzes exported methods on a struct and generates a clean interface definition. |
...
| `add_struct_tags` | `file_path` (string, required)<br>`struct_name` (string, required)<br>`tags` (string, optional) | Appends or updates struct field tags, automatically converting `CamelCase` field names to `snake_case`. |
| `tidy_imports` | `file_path` (string, required) | Organizes, groups, and cleans unused imports in a file (equivalent to `goimports`). |
