...
func MoveFile(opts MoveFileOptions) error {
	return moveFileImpl(opts.SourceFile, opts.DestDir)
}

func moveFileImpl(sourceFile, destDir string) error {
	...
	if strings.HasSuffix(baseName, "_test.go") {
		implName := strings.TrimSuffix(baseName, "_test.go") + ".go"
		candidate := filepath.Join(sourceDir, implName)
